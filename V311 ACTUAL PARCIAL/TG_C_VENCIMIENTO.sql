...
-- ********************************************************************************
-- NUEVO TRIGGER PARA 90xx CREADO POR: JOSE RAVELO ABR-2018 
-- AVISA AL USUARIO QUE ESTA CERCA DE LA FECHA DE VENCIMIENTO DE LICENCIA CONCESIONARIO.
-- 
-- ********************************************************************************
 
if exists (select * from dbo.sysobjects where id = object_id(N'[AVISO_VENCIMIENTO_LICENCIA]'))
drop TRIGGER AVISO_VENCIMIENTO_LICENCIA  
GO
 

CREATE TRIGGER TG_C_MASTER ON SACONF
-- WITH ENCRYPTION
FOR UPDATE  
AS
-----------------------------------
