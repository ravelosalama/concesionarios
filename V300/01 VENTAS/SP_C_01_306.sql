...
if exists (select * from dbo.sysobjects where id = object_id(N'[SP_C_01_306]'))
drop PROCEDURE SP_C_01_306  
GO
...
CREATE PROCEDURE  [dbo].[SP_C_01_306] @TipoDoc varchar(1), @NumeroD Varchar(10), @CodTercero Varchar(10), @Codusua Varchar(10),@Codoper Varchar(10)
--WITH ENCRYPTION
AS
...
--===================================================
-- 306 - INICIO ANULACION DE PRESUPUESTO A TALLER 
--===================================================

IF @TipoDoc='F' AND @Codoper='01-306' 
...
--------------------- 
   ...
   -- 5, 6 Y 7. Valida OR citada en el vale exista en BD, CodClie valido y Satatus pendiente. 
    ...
    BEGIN
      ...
      IF  @CodTercero<>'99008' -- VALIDA CLIENTE - TRANSACCION VALIDA.
       BEGIN
          SET @STATUSERROR=1
...
IF @STATUSERROR=0 
 ...
 SELECT @CODASESOR=CODVEND FROM SAFACT WHERE NumeroD=@OR AND TipoFac='G'
  SET @DESCRIPERROR='TOY AQUI CON:'+@ONumero--+@CODASESOR+@NUMEROD+@MODELO+@ONUMERO
 ...
 UPDATE SAFACT SET DESCRIP='ANULACION PRESUPUESTO:'+@ONUMERO+' '+@CODASESOR+'/'+@OR,OrdenC=@OR, NOTAS4=@SERIAL+' '+@MODELO WHERE NUMEROD=@NumeroD AND TIPOFAC='F'
 UPDATE SAFACT SET DESCRIP='PRESUPUESTO ANULADO:'+@NUMEROD+' '+@CODASESOR+'/'+@OR,OrdenC=@OR, NOTAS4=@SERIAL+' '+@MODELO WHERE NUMEROD=@ONUMERO AND TIPOFAC='F'
