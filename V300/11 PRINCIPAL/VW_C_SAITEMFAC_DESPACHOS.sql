...
-- RESUELVE
-- QUE OBJETOS: REPORTE 302 - AUDITORIA OR DESPACHOS
-- HECHO POR:JOSE RAVELO  FECHA:SEP 2016
  
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VW_C_SAITEMFAC_DESPACHOS]'))
...
SELECT   X.CodItem,X.Descrip1,X.Cantidad,Y.Nro_OR,Z.CodOper,X.NumeroD,X.TipoFac,Z.NumeroR,Z.FechaE,Y.vale
